...
class Generator:
    ...
    def generate_class(
        ...
        with m.class_(clsname, "Visitor"):
            ...
            if links:
                self._gen_properties_visitors(ev, m=m, links=links)
                self._gen_properties_visitor_mapping(ev, m=m, links=links)

            # pattern properties
            if self.helper.has_pattern_properties(ev):
                links = ev.get_annotated(names.annotations.pattern_properties_links)
                self._gen_pattern_properties_visitor_pairs(ev, m=m, links=links)
    ...
    def _gen_properties_visitor_mapping(self, ev: Event, *, m, links) -> None:
        m.stmt("@reify")
        with m.def_("_properties_visitor_mapping", "self"):
            m.stmt("return {")
            with m.scope():
                for name, uid in links:
                    m.stmt(f"{name!r}: self.{self.helper.methodname(name)},")
            m.stmt("}")

    def _gen_pattern_properties_visitor_pairs(self, ev: Event, *, m, links) -> None:
        m.stmt("@reify")
        with m.def_("_pattern_properties_visitor_pairs", "self"):
        ...
        with m.def_("_visit", "self", "ctx: Context", "d: dict"):
            ...
            if self.helper.has_additional_properties(ev):
                ...
                with m.for_("k, v in d.items()"):
                    ...
                    if self.helper.has_pattern_properties(ev):
                        with m.for_(
                            "rx, visitor in self._pattern_properties_visitor_pairs"
                        ):
    ...
    def _gen_properties_visitors(self, ev: Event, *, m, links) -> None:
        for name, uid in links:
            if self.helper.is_link_of_anonymous_definition(uid):
